...
| `env_var` | Conditional | - | Explicit environment variable name for the phantom token. Required when `credential_key` is `op://`, `bw://`, `apple-password://`, `keyring://`, `file://`, or `cmd://`. Optional for `env://` (derived from the URI) |
...
When `interaction.open_urls` is present, nono gives that capture command a temporary browser helper. Opens routed through `BROWSER` or the macOS `open` shim are validated against the capture entry, independent of the command sandbox's top-level `open_urls`. Non-URL `open` fallback through the shim is available only when `allow_launch_services` is true.
...
- **Credential key must be alphanumeric** (letters, numbers, and underscores only) unless it uses a supported URI scheme such as `op://`, `bw://`, `apple-password://`, `keyring://`, `file://`, `env://`, or `cmd://`
