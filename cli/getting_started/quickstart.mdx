...
The `why` command checks whether a filesystem path, network operation, kernel scope, or command-policy invocation would be allowed or denied. It's designed for both human debugging and programmatic use by AI agents.
...
| Flag | Description |
|------|-------------|
| `--command` | Command-policy command name to check |
